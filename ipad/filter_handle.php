...
function searchmyAccountList($db, $filterId,$stationId ,$stageId)
{
    ...
    $where =" where 1 and  `product_id`='14' and( `user_id`='$user_id' or `user_id`='1' )";
    ...
    $account_select_count_select = "SELECT count(1) as num from filter where `id`='$filterId' and `product_id`='14'";
...
function getFilterList($db, $userId)
{
    ...
    $where .="and f.product_id='14' and (f.user_id = '$userId' or f.user_id = '1') order by `user_id` desc ,id asc";
...
{   
    ...
    $filter_build_sql = "select a.id,a.build_id,a.station_id from `filter` as a  where (`user_id` = '$userId'  or `user_id` = '1') and `product_id` = '14' $where1 ";
        ...
        // $product = explode("|", $filter_select_result[0]['product_id']);
            ...
            //   $user_id=$_SESSION['cooper_user_info'][0]['id'];
              ...
              if($user_project==""||$user_project==NULL){
                  $project_select_sql = "SELECT `project_id` from `product_project` where `enable` = '1' and `product_id` = '14'";
                  $project_select_result = $db->query($project_select_sql);
            ...
            $product_info_sql = "SELECT `product` from `product` where `enable` = '1' and `id` = '14' ";
