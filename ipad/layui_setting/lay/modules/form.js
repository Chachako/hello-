            ...
            select: function(){
                    ...
                    ,events = function(reElem, disabled, isSearch){
                            ...
                            if (isMulti) {
                                ...
                                if(othis.find("input[type='checkbox']").is(':checked')){
                                    if (omit) {
                                        ...
                                        input.eq(0).val("Selected "+othis.parent().find('[type=checkbox]:checked').length+" items");
                                    }
                                    valueStr.push(value);
                                }else{
                                    if (omit) {
                                        ...
                                        if (num == 0) {
                                            ...
                                            input.eq(0).val("Selected "+num+" items");
                                        }
                                        valueStr.removeElem(value);
                                    }
                        ...
                        function handleMultiOption () {
                            ...
                            if (omit) {
                                ...
                                input.eq(0).val(selectedOption.length==0?"":"Selected "+selectedOption.length+" items");
                            }
                ...
                selects.each(function(index, select){
                    ...
                    var isSearch = typeof othis.attr('lay-search') === 'string'
                        ...
                        ,inputValue =  !(typeof $(select).attr("lay-omit") === 'undefined')&&value!=null&&value.length>0 ? 'Selected '+value.length+" items" : "";
                    ...
                    if (isMulti) {
                        var reElem = $(['<div class="' + (isMulti ? '' : 'layui-unselect ') + CLASS + (disabled ? ' layui-select-disabled' : '') + '">'
                            ...
                            , '<dl class="layui-anim layui-anim-upbit' + (othis.find('optgroup')[0] ? ' layui-select-group' : '') + '" '+ ((isSearch || isTools)?'style="overflow-y: hidden;"':'') +'>' + function (options) {
                                var arr = [], height=247 ,tools = '<dd class="layui-select-tips"><div class="multiOption" data-value="all"><i class="iconfont icon-quanxuan"></i>Select all</div><div class="multiOption" data-value="none"><i class="iconfont icon-qingkong"></i>Empty</div></dd>';
