...
function task_select_user($db)
{
    ...
    $select_sql = "select product_id,project_id,check_user from team_watch.issue_list where `id`='$task_id'";
...
function task_assign_user($db)
{
    ...
    // $sql = "select * from team_watch.issue_list where id='$task_id'";
    ...
    $sql="update team_watch.issue_list set check_user='$check_user' where id='$task_id' ";
    $res=$db->execSql($sql);
    if($res){
        $msg='success';
        return $msg;
...
function task($db){
    ...
    $sql="update team_watch.issue_list set access_user_id='$access_user_id' where id='$task_id'";
    $db->query($sql);
    }
    ...
    // (select a.*,pd.product from team_watch.issue_list as a LEFT JOIN product as pd on a.product_id=pd.id) as b LEFT JOIN project as pj on b.project_id=pj.id) as c
...
//     (select * from team_watch.history where issue_list_id= '$task_id' order by id) as b on u.id=b.user_id) as c on c.assign=g.id) as d LEFT JOIN user_list as e ON
...
LEFT JOIN `group` as gp on u.group_id=gp.id where b.issue_list_id= '$task_id' ";
    // var_dump($sql);die;
    if($task[0]['step'] == '3')
        $sql.=" and b.id != (select b.* from (select id from team_watch.history where issue_list_id='$task_id' order by id desc limit 1) as b) order by id";
    else
        $sql.=" order by id";
...
function task_update($db){
    ...
    if($task_group_id==$group_id){//若当前用户的部门和当前task所在的部门为同一个，则执行插入
        ...
        return $msg;
            ...
            if($version!=""&&$group_id==10){
                $sql="update team_watch.issue_list set `step`='1',`assign`='$assign',access_user_id='$user_id',status_id='1',check_user='$new_check_user' ,version='$version',new_status='$status' where id='$task_id'";
               
            }else{
                 $sql="update team_watch.issue_list set `step`='1',`assign`='$assign',access_user_id='$user_id',status_id='1',check_user='$new_check_user',new_status='$status'  where id='$task_id'";
            }
     ...
     if($assign==13)
     {
        $sql="insert into team_watch.history (`issue_list_id`,`user_id`,`update_time`,`status`,`upload`,`oldupload`,`assign`,`assign_user`,`version`) values 
    ...
    function task_block($db){
        ...
        return $msg;
             ...
             if($version!=""&&$group_id==10){
                $sql="update team_watch.issue_list set `step`='2',access_user_id='$user_id',status_id='2' ,version='$version',new_status='$status' where id='$task_id'";
             }else{
                 $sql="update team_watch.issue_list set `step`='2',access_user_id='$user_id',status_id='2',new_status='$status' where id='$task_id'";
             }
        ...
        function task_cancel($db){
            ...
            if(empty($status) && empty($file)){
                //     if($version==""){
                // $sql="update team_watch.issue_list set `step`='3',cancel_content='$cancel_content',status_id='3',cancel_user='$user',cancel_done_time='$cancel_done_time' where id='$task_id'";
                //     }else{
                // $sql="update team_watch.issue_list set `step`='3',cancel_content='$cancel_content',status_id='3',cancel_user='$user',cancel_done_time='$cancel_done_time',version='$version'  where id='$task_id'";        
                //     }
            ...
            $sql="insert into team_watch.history (`issue_list_id`,`user_id`,`update_time`,`status`,`upload`,`oldupload`) values 
            ...
            if(empty($status) && empty($file)){
                    ...
                    if($version!=""&&$group_id==10){
                $sql="update team_watch.issue_list set `step`='3',cancel_content='$cancel_content',status_id='3',cancel_user='$user',cancel_done_time='$cancel_done_time',version='$version' 
            ...
            $sql="insert into team_watch.history (`issue_list_id`,`user_id`,`update_time`,`status`,`upload`,`oldupload`,`version`) values 
            ...
            $db->query($sql);
             ...
             if($version!=""&&$group_id==10){
                ...
                $sql="update team_watch.issue_list set `step`='3',cancel_content='$cancel_content',status_id='3',cancel_user='$user',cancel_done_time='$cancel_done_time' 
                  where id='$task_id'";
            ...
            function task_done($db){
                ...
                if(empty($status) && empty($file)){
                    if($version!=""&&$group_id==10){
                    $sql="update team_watch.issue_list set `step`='4',done_content='$done_content',done_score='$done_score',status_id='4',done_user='$user',
            ...
            function update_eta($db){
                ...
                $sql="update team_watch.issue_list set `eta`='$update_eta' where id='$task_id'";
                $db->query($sql);
                $msg='success';
                return $msg;
            }
            ...
            function clear_message($db){ 
                ...
                for ($i=0; $i < count($sql_result); $i++) { 
                    ...
                    if(!in_array($userId, $access_user_id)){
                        ...
                        $update_sql = "update team_watch.issue_list set access_user_id='$access_user_id' where id='$task_id'";
