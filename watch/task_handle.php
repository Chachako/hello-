...
{   
    ...
    // $limit = $post['limit'];
       ...
       $task_select_sql = "SELECT a.`id`,a.`task_title`,a.`task_description`,a.`eta`,a.`version`,a.`new_status`,a.`cancel_user`,a.`done_user`,b.`product`,c.`project`,d.`build`,e.`station`,f.`status`,a.`cancel_content`,u.`username` ,us.`username` as `toUser`,a.`priority` , ul.`username` as requestor  from team_watch.issue_list as a
       ...
       ";
       $task_count_select_sql = "SELECT count(a.`id`) as num from team_watch.issue_list as a";
       ...
       if ($group_task) {
           ...
           if ($depId!=0&&$depId!='') {
               $temp_sql .= " and exists (select 1 from (select h.id from team_watch.issue_list as h left join user_list as i on h.check_user = i.id where i.group_id = '$depId' and h.`done_user` is null and h.`cancel_user` is null union
...
{   
    $userId = $post['userId'];
    $setting_select_result = chooseTable($db, $userId, $choose);
    $task_select_sql = "SELECT a.`id`,a.`task_title`,a.`task_description`,a.`new_status`,a.`eta`,a.`version`,a.`cancel_user`,a.`done_user`,b.`product`,c.`project`,d.`build`,e.`station`,f.`status`,a.`cancel_content`,u.`username` ,us.`username` as `toUser`,a.`priority` , ul.`username` as requestor  from team_watch.issue_list as a
    left join `product` as b on a.`product_id` = b.`id`
    left join `project` as c on a.`project_id` = c.`id`
    left join `build` as d on a.`build_id` = d.`id`
    left join `station` as e on a.`station_id` = e.`id`
    ...
    $task_select_sql .= $limit_sql;
    $task_select_result = $db->query($task_select_sql);
    // $history_select_sql1="SELECT * FROM (SELECT `id`,`issue_list_id`,`status` FROM team_watch.history ORDER BY `update_time` DESC)b1 GROUP BY `issue_list_id`  ";
...
function taskFilterList($db, $post, $filterId,  $depId,$stageId,$stationId)
{
    ...
    u.`username` ,us.`username` as `toUser`, a.`priority`, ul.`username` as requestor  from team_watch.issue_list as a
    left join `product` as b on a.`product_id` = b.`id`
    left join `project` as c on a.`project_id` = c.`id`
    left join `build` as d on a.`build_id` = d.`id`
    left join `station` as e on a.`station_id` = e.`id`
...
select h.id from team_watch.issue_list as h left join user_list as i on h.`cancel_user` = i.`username` where i.group_id = '$depId') as j where j.id=a.id) ";
    ...
    for ($i = 0; $i < count($task_select_result); $i++) {
        ...
        $history_status_sql = "SELECT `status` FROM team_watch.history WHERE `issue_list_id` = '" . $task_select_result[$i]['id'] . "' ORDER BY `id` DESC";
