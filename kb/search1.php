...
$stage_sql="select id from `user_build` where `user_id`='$user_id' and `product_id`='17'";
...
if($stage_task){
    ...
    $stage_insert = "INSERT INTO `user_build`(`user_id`,`product_id`,`build_id`) values('$user_id','17','$stageId')";
...
//     }else{
//         $issue_id=$value['id'];
//         $sql="select a.*,u.username,g.group from team_keyboard.history as a left join user_list as u on u.id=a.user_id left join `group` as g on a.assign=g.id where a.issue_list_id= '$issue_id' order by a.id desc";
