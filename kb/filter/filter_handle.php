...
$username = $_SESSION['cooper_user_info'][0]['username'];
...
function searchmyAccountList($db,$stageId,$stationId)
{
    ...
    $account_select_sql = "SELECT `id`,`user_id`,`filter_name`,`product_id`,`project_id`,`build_id`,`station_id`,`status_id`,`start_time`,`end_time` FROM `filter` where `user_id`='$user_id' and `product_id`='17'  ";
    ...
    $where .="and f.product_id='17' and f.user_id=' $user_id' ";
...
// echo $account_select_sql;die;
    ...
    $account_select_count_select = "SELECT count(1) as num from filter where `user_id`='$user_id' and `product_id`='17'";
    $account_select_count_result = $db->query($account_select_count_select,'Row');
...
function filterAllListInit($db, $post, $choose)
{
   ...
   $task_select_sql = "SELECT a.`id`,a.`task_title`,a.`task_description`,a.`eta`,a.`cancel_user`,a.`done_user`,b.`product`,c.`project`,d.`build`,e.`station`,f.`status`,a.`cancel_content`,u.`username` ,us.`username` as `toUser`,a.`priority` , ul.`username` as requestor  from team_ipad.issue_list as a
    ...
    $station_sql="select station_id from `user_station` where `user_id`='$userId' and `product_id`='17'";
...
function taskFilterList($db, $post, $filterId, $stageId, $depId)
{
    ...
    $stage_sql="select id from `user_build` where `user_id`='$userId' and `product_id`='17'";
    $stage_task=$db->query($stage_sql);
    if($stage_task){
        ...
        $stage_insert = "INSERT INTO `user_build`(`user_id`,`product_id`,`build_id`) values('$userId','17','$stageId')";
