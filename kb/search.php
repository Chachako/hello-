...
function search_all_task($db,$depId,$stageId,$stationId){
    ...
    $group_sql="select id from `user_group` where `user_id`='$user_id' and `product_id`='17'";
    ...
    if($group_task){
        ...
        $group_insert = "INSERT INTO `user_group`(`user_id`,`product_id`,`group_id`) values('$user_id','17','$depId')";
        $group_insert_task = $db->execSql($group_insert);
    }

    if($depId != 0)
        $where .= " and exists (select 1 from (select h.id from team_keyboard.issue_list as h left join user_list as i on h.check_user = i.id where i.group_id = '$depId' and h.`done_user` is null and h.`cancel_user` is null union
...
select h.id from team_keyboard.issue_list as h left join user_list as i on h.`cancel_user` = i.`username` where i.group_id = '$depId') as j where j.id=a.id) ";
    ...
    $count_sql="select count(id) as count from team_keyboard.issue_list as a $where";
...
select h.id from team_keyboard.issue_list as h left join user_list as i on h.`cancel_user` = i.`username` where i.group_id = '$depId') as j where j.id=a.id) ";
     ...
     $sql="select DISTINCT(issue_list_id) from team_keyboard.history where user_id='$user_id'";
     $ids=$db->query($sql);
    ...
    $sql .= $limit_sql;
    $task=$db->query($sql);
    $count_sql="select count(id) as count from team_keyboard.issue_list as a $where and (a.create_user_id='$user_id' or a.check_user='$user_id')";
    $count=$db->query($count_sql);
    $count=$db->query($count_sql);
    $task_count=count($task);
       for ($i = 0; $i <  $task_count; $i++) {
...
function search_all_filter($db,$stageId,$stationId,$userId){
    ...
    session_start();
    $project_id=$_SESSION['cooper_user_info'][0]['project_id'];
    $where = " where 1 and( f.user_id='1' or f.user_id='$userId') and product_id='17' ";
    $where1="  where 1 ";
    if($project_id!=''&&$project_id!=NULL){
        $project_id=explode('|', $project_id) ;
        for ($i = 0; $i < count($project_id); $i++) {
...
//         $a=stristr($stageId,"|");
//         if($a!=false){
//             $where .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and build_id like '%$stageId%'";

//         }else{
//             $where .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and FIND_IN_SET('$stageId',replace(f.build_id, '|',','))";
//         }
//         $countWhere = $where;
//         $account_select_sql = "SELECT f.* FROM `filter` as f $where ";
//         $account_select_result = $db->query($account_select_sql);
...
//             if($b!=false){
//                 if($a!=false){
//                     $where1 .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and station_id like '%$stationId%' and build_id like '%$stageId%'";
//                 }else{
//                     $where1 .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and FIND_IN_SET('$stageId',replace(f.build_id, '|',',')) and station_id like '%$stationId%'";
//                 } 
//             }else{
//                 if($a!=false){
//                     $where1 .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and FIND_IN_SET('$stationId',replace(f.station_id, '|',',')) and build_id like '%$stageId%'";
//                 }else{
//                     $where1 .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and FIND_IN_SET('$stationId',replace(f.station_id, '|',',')) and FIND_IN_SET('$stageId',replace(f.build_id, '|',','))";
//                 }
               
//             }
//         //    $where1 .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and build_id like '%$stageId%'  and station_id like '%$stationId%' ";
       
//        $countWhere = $where1;
//        $account_select_sql = "SELECT f.* FROM `filter` as f  $where1";
//        $account_select_result = $db->query($account_select_sql);
//        $account_select_count_select = "SELECT count(1) as num from filter as f $countWhere";
//        $account_select_count_result = $db->query($account_select_count_select,'Row');   
//    }
...
//         $a=stristr($stationId,"|");
//         if($a!=false){
//             $where .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and station_id like '%$stationId%'";
//         }else{
//             $where .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' and FIND_IN_SET('$stationId',replace(f.station_id, '|',','))";
//         }
//         $countWhere = $where;
...
//         $where .="and( f.user_id='1' or f.user_id='$userId') and product_id='17' ";
//         $countWhere = $where;
//         // $where .="limit $page,$limit ";
//         $account_select_sql = "SELECT f.* FROM `filter` as f  $where";
//         $account_select_result = $db->query($account_select_sql);
