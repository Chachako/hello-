...
{ $project_sql = "select project_id from `user_list` where `id` = '$userId'";
    ...
    $filter_build_sql = "select a.id,a.build_id,a.station_id from `filter` as a  where (`user_id` = '$userId' and `product_id` = '17' ) or `user_id` = '1'  $where1 ";
...
function searchmyAccountList($db, $filterId, $stationId, $stageId)
{
    ...
    $where =" where 1 and  `product_id`='17' and( `user_id`='$user_id' or `user_id`='1' )";
    ...
    $account_select_count_select = "SELECT count(1) as num from filter where `id`='$filterId' and `product_id`='17'";
...
function getFilterList($db, $userId)
{
    ...
    $where .="and f.product_id='17' and (f.user_id = '$userId' or f.user_id = '1') order by `user_id` desc ,id asc";
