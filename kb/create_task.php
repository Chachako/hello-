...
function create_search($db)
{
    ...
    // }
    $product_sql="select * from `product` where enable='1' and product='keyboard'";
    $product_res=$db->query($product_sql);
    $product= $product_res;
    $where="";
...
function create_task($db){
    ...
    foreach($station as $key=>$value){
        ...
        $pj_issue_sql="SELECT no from team_keyboard.issue_list where `project_id`='$project_id' order by id desc ";
        ...
        $sql="insert into team_keyboard.issue_list (no,create_user_id,product_id,project_id,build_id,
