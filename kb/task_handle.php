...
{   
    ...
    // $limit = $post['limit'];
       ...
       $task_select_sql = "SELECT a.`id`,a.`task_title`,a.`task_description`,a.`eta`,a.`version`,a.`new_status`,a.`cancel_user`,a.`done_user`,b.`product`,c.`project`,d.`build`,e.`station`,f.`status`,a.`cancel_content`,u.`username` ,us.`username` as `toUser`,a.`priority` , ul.`username` as requestor  from team_keyboard.issue_list as a
       ...
       ";
       $task_count_select_sql = "SELECT count(a.`id`) as num from team_keyboard.issue_list as a";
       ...
       $group_sql="select group_id from `user_group` where `user_id`='$userId' and `product_id`='17'";
       $group_task=$db->query($group_sql);
       if ($group_task) {
           $depId = $group_task[0]['group_id'];
           if ($depId!=0&&$depId!='') {
               $temp_sql .= " and exists (select 1 from (select h.id from team_keyboard.issue_list as h left join user_list as i on h.check_user = i.id where i.group_id = '$depId' and h.`done_user` is null and h.`cancel_user` is null union
   select h.id from team_keyboard.issue_list as h left join user_list as i on h.`done_user` = i.`username` where i.group_id = '$depId' union
   select h.id from team_keyboard.issue_list as h left join user_list as i on h.`cancel_user` = i.`username` where i.group_id = '$depId') as j where j.id=a.id) ";
           }
       }
...
{   
    $userId = $post['userId'];
    $setting_select_result = chooseTable($db, $userId, $choose);
    $task_select_sql = "SELECT a.`id`,a.`task_title`,a.`task_description`,a.`new_status`,a.`eta`,a.`version`,a.`cancel_user`,a.`done_user`,b.`product`,c.`project`,d.`build`,e.`station`,f.`status`,a.`cancel_content`,u.`username` ,us.`username` as `toUser`,a.`priority` , ul.`username` as requestor  from team_keyboard.issue_list as a
    left join `product` as b on a.`product_id` = b.`id`
    left join `project` as c on a.`project_id` = c.`id`
    left join `build` as d on a.`build_id` = d.`id`
    left join `station` as e on a.`station_id` = e.`id`
    ...
    $task_select_sql .= $limit_sql;
    $task_select_result = $db->query($task_select_sql);
    // $history_select_sql1="SELECT * FROM (SELECT `id`,`issue_list_id`,`status` FROM team_keyboard.history ORDER BY `update_time` DESC)b1 GROUP BY `issue_list_id`  ";
...
function taskFilterList($db, $post, $filterId,$depId,$stageId,$stationId)
{
    ...
    u.`username` ,us.`username` as `toUser`, a.`priority`, ul.`username` as requestor  from team_keyboard.issue_list as a
    left join `product` as b on a.`product_id` = b.`id`
    left join `project` as c on a.`project_id` = c.`id`
    left join `build` as d on a.`build_id` = d.`id`
    left join `station` as e on a.`station_id` = e.`id`
    ...
    for ($i = 0; $i < count($stationId); $i++) {
     ...
     $where='';
    $group_sql="select id from `user_group` where `user_id`='$userId' and `product_id`='17'";
    $group_task=$db->query($group_sql);
    if($group_task){
        ...
        $group_insert = "INSERT INTO `user_group`(`user_id`,`product_id`,`group_id`) values('$userId','17','$depId')";
...
select h.id from team_keyboard.issue_list as h left join user_list as i on h.`cancel_user` = i.`username` where i.group_id = '$depId') as j where j.id=a.id) ";
    ...
    for ($i = 0; $i < count($task_select_result); $i++) {
        ...
        // $history_status_sql = "SELECT `status` FROM team_keyboard.history WHERE `issue_list_id` = '" . $task_select_result[$i]['id'] . "' ORDER BY `id` DESC";
...
function getStageStationlist($db, $userId)
{
    ...
    $pro_sql = "select project_id from `product_project` where `product_id` = '17' and `enable` = '1' $where";
    ...
    $user_build_sql = "select build_id from `user_build` where `user_id` = '$userId' and `product_id` = '17'";
    ...
    $user_station_sql = "select station_id from `user_station` where `user_id` = '$userId' and `product_id` = '17'";
...
{
    // TODO 暂未考虑翻页
    $group_sql="select * from `group` where 1 and (`product_id` like '17' or `product_id` like '17|%' or `product_id` like '%|17|%' or `product_id` like '%|17') and `enable` = '1'";
    $group_res=$db->query($group_sql);
    $select_user_dep = "select * from user_group where `product_id` = '17' and `user_id` = '$userId'";
...
function taskFilter($db, $post, $userId)
{
    ...
    $where="";
    $stage_sql="select build_id from `user_build` where `user_id`='$userId' and `product_id`='17'";
    $stage_task=$db->query($stage_sql);
    if($stage_task){
...
function chooseTable($db, $userId, $choose)
{
    ...
    $choose_select_sql = "SELECT `id`,`product_id`,`project_id`,`build_id`,`station_id`,`status_id` from `$choose` where product_id = 17 and `user_id` = '$userId' ";
